...
board stringToBoard(std::string fileInfo){
	...
	if(container.size() != 1){
		std::cerr << "Error with the terrain portion" << '\n';
		throw -1;
	}
	smatch xrange;
	regex_search(container[0].str(), xrange, reXrange);
	if(xrange.size() != 1){
		std::cerr << "Error with the Xrange portion of terrain" << '\n';
		throw -1;
	}
	...
	if(xrange.size() != 1){
		std::cerr << "Error with the Yrange portion of terrain" << '\n';
		throw -1;
	}
	...
	if((sscanf(x.c_str(), "%d..%d", &xlow, &xhigh)) != 2){
		std::cerr << "Error reading range of x values" << '\n';
		throw -1;
	}
	if((sscanf(y.c_str(), "%d..%d", &ylow, &yhigh)) != 2){
		std::cerr << "Error reading range of y values" << '\n';
		throw -1;
	}
	...
	if(container.size() != 1){
		std::cerr << "Error with the Chars portion" << '\n';
		throw -1;
	}
	...
	if(aliveLoc){
		if((err = sscanf(aliveLoc, "Alive=%d", &alive)) != 1){
			std::cerr << "Error reading Alive value from Chars portion, default will be used" << '\n';
			alive = 64;
		}
	}else{
		std::cerr << "Could not find location of Alive value, default will be used" << '\n';
		alive = 64;
	}
	char * deadLoc = strstr(container.str().c_str(), "Dead=");
	if(deadLoc){
		if((sscanf(deadLoc, "Dead=%d", &dead)) != 1){
			std::cerr << "Error reading Dead value from Chars portion. default will be used" << '\n';
			dead = 126;
		}
	}else{
		std::cerr << "Could not find location of Dead value, default will be used" << '\n';
		dead = 126;
	}
	...
	if(container.size() == 1){
		...
		if((sscanf(x.c_str(), "%d..%d", &xlow, &xhigh)) != 2){
			std::cerr << "Error reading range of x values, default will be used" << '\n';
			xlow = gameBoard.getXMin();
			xhigh = gameBoard.getXMax();
		}
		if(xlow < gameBoard.getXMin()){
			std::cerr << "Window X-Low was out of range of the terrain, default will be used\n";
			xlow = gameBoard.getXMin();
		}
		if(xhigh > gameBoard.getXMax()){
			std::cerr << "Window X-High was out of range of the terrain, default will be used\n";
			xhigh = gameBoard.getXMax();
		}
		if((sscanf(y.c_str(), "%d..%d", &ylow, &yhigh)) != 2){
			std::cerr << "Error reading range of y values" << '\n';
		...
		if(ylow < gameBoard.getYMin()){
			std::cerr << "Window Y-Low was out of range of the terrain, default will be used\n";
			ylow = gameBoard.getYMin();
		}
		if(yhigh > gameBoard.getYMax()){
			std::cerr << "Window Y-High was out of range of the terrain, default will be used\n";
	...
	if(container.size() != 1){
		std::cerr << "Error with the Colors portion" << '\n';
		throw -1;
	}
	smatch aliveMatch;
	smatch deadMatch;
	...
	if(aliveMatch.size() == 1){
		...
		if(aliveMatch.size() != 1){ 
			std::cerr << "Error attempting to read Alive color value from Colors portion, default will be used" << '\n';
			gameBoard.setAliveColor(255, 255, 255);
		}
		aliveColor = aliveMatch.str();
		int r, b ,g;
		if((sscanf(aliveColor.c_str(), "(%d,%d,%d)", &r, &g, &b)) != 3){
			std::cerr << "RBG value for Alive in Colors portion was not valid, default will be used" << '\n';
			gameBoard.setAliveColor(255, 255, 255);
		}
		...
		std::cerr << "Alive portion not found, default color will be used\n";
		gameBoard.setAliveColor(255, 255, 255);
	}
	...
	if(deadMatch.size() == 1){
		...
		if(deadMatch.size() != 1){
			std::cerr << "Error attempting to read Dead color value from Colors portion, default will be used" << '\n';
			gameBoard.setDeadColor(64 , 64, 64);
		}
		deadColor = deadMatch.str();
		if((sscanf(deadColor.c_str(), "(%d,%d,%d)", &r, &g, &b)) != 3){
			std::cerr << "RBG value for Dead in Colors portion was not valid, default will be used." << '\n';
			gameBoard.setDeadColor(64 , 64, 64);
		}
		else if(r < 0 || r > 255 || b < 0 || b > 255 || g < 0 || g > 255){
			std::cerr << "RBG value for Dead in Colors portion was not valid" << '\n';
			gameBoard.setDeadColor(64 , 64, 64);
		}else{
			gameBoard.setDeadColor(r, g, b);
		}
	}else{
		std::cerr << "Dead color not found default will be used\n";
	...
	if(container.size() != 1){
		std::cerr << "Error with Initial portion\n";
		throw -1;
	}
	...
	while(regit != end){
		...
		if((sscanf(regit->str().c_str(), "(%d, %d)", &xc, &yc)) != 2){
			std::cerr << "Error reading X, Y coordinate" << '\n';
			continue;
		}
		if(xc < xlow || xc > xhigh || yc < ylow || yc > yhigh){
			std::cerr << "Coordinate in " << regit->str() << " was not in range of the terrain." << '\n';
			continue;
		}
...
std::string getStringFromStdIn(){
	try{
		...
		std::cerr << "Error getting string from stdin\n";
		throw -1;
	}
}
